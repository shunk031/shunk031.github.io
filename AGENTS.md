...
- 対応表: Recent & Upcoming Talks の更新・外部ソース差分確認・talks 同期は `hugo-talks-sync` skill を先に使い、候補の event ページ作成は `hugo-event-intake` skill に渡してください。
- 単発の event ページ追加は `hugo-event-intake` skill、publication のサムネイル生成は `hugo-publication-thumbnail` skill、ポスターの Projects 登録は `hugo-poster-project-sync` skill、採択・発表 news の同期は `hugo-conference-news-sync` skill を使ってください。呼び出しコマンドは各 skill の `SKILL.md` を参照し、conference news 同期は top-level の `scripts/sync_conference_news.py` を使います (スキル配下に同名スクリプトがあっても top-level 版が正です)。
