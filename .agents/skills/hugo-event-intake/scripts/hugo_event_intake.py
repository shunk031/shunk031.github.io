...
class MetadataHTMLParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        elif "speakerdeck-embed" in attr_map.get("class", "") and not self.speakerdeck_embed_id:
...
def infer_kind(notes: list[str], sources: list[SourceInfo], explicit_kind: str = "") -> str:
    ...
    if any(keyword in haystack for keyword in ("journal club", "reading", "paper reading", "論文読み会", "読み会", "輪読")):
        return "Journal Club"
...
def resolve_source_url(
    ...
    *,
    ...
    if is_speakerdeck_url(url):
        return resolve_speakerdeck_source(url, fetch_text_response=fetch_text_response, fetch_json=fetch_json)
...
def resolve_speakerdeck_source(
    ...
    *,
    ...
    raw_title = str(oembed.get("title", "") or parser.meta.get("og:title") or parser.title).strip()
    ...
    return SourceInfo(
        ...
        embed_ratio=parser.speakerdeck_embed_ratio or str(oembed.get("ratio", "") or ""),
    )
...
def derive_slug(explicit_slug: str, sources: list[SourceInfo], title_core: str, event_name: str, date: str) -> str:
...
def build_body_markdown(spec: EventSpec, *, description: str, speakerdeck_source: SourceInfo | None) -> str:
    parts: list[str] = []
    if spec.event:
        parts.append(f"# {spec.event}\n")
    if description.strip():
        quote_lines = [f"> {line.strip()}" if line.strip() else ">" for line in description.strip().splitlines()]
        parts.append("\n".join(quote_lines) + "\n")
...
def probe_event(
    ...
    *,
    ...
    notes = [value.strip() for value in inputs if value.strip() and not looks_like_url(value)]
    ...
    speakerdeck_source = next((source for source in sources if source.source_type == "speakerdeck"), None)
    html_source = next((source for source in sources if source.source_type == "html"), None)
    pdf_source = next((source for source in sources if source.source_type == "pdf"), None)
    video_source = next((source for source in sources if source.source_type == "video"), None)
    ...
    if not raw_title:
        ...
        if not raw_title and pdf_source:
            raw_title = Path(urlsplit(pdf_source.url).path).stem.replace("-", " ").strip()

    event_hint = event.strip()
    title_core = raw_title.strip()
    if speakerdeck_source and speakerdeck_source.title:
        source_event_hint, cleaned_title, _ = split_prefixed_title(speakerdeck_source.title)
    ...
    if not event_hint and html_source and html_source.title and html_source.title != title_core:
    ...
    final_date_end = date_end.strip() or (html_source.date_end if html_source and html_source.date_end else final_date)
    ...
    final_address = normalize_address(
        {
            "street": street.strip() or (html_source.address["street"] if html_source else ""),
            "city": city.strip() or (html_source.address["city"] if html_source else ""),
            "region": region.strip() or (html_source.address["region"] if html_source else ""),
            "postcode": postcode.strip() or (html_source.address["postcode"] if html_source else ""),
            "country": country.strip() or (html_source.address["country"] if html_source else ""),
        }
    )
    final_url_slides = url_slides.strip() or (speakerdeck_source.url if speakerdeck_source else "")
    ...
    for field_name in ("title", "event", "date", "date_end", "location", "featured_source_url"):
...
def create_pdf_thumbnail(pdf_path: Path, output_path: Path) -> Path:
    ...
    if shutil.which("magick"):
        ...
        command = "convert"
    if not command:
        raise FileNotFoundError("ImageMagick command not found: expected `magick` or `convert`.")
    subprocess.run(
        [
            command,
            f"{pdf_path}[0]",
...
def write_event(
    ...
    *,
    ...
    if spec.featured_source_url:
        write_featured_asset(event_dir, spec.featured_source_url, download_binary=download_binary)
    index_path.write_text(render_frontmatter(spec) + spec.body_markdown.strip() + "\n", encoding="utf-8")
    return index_path


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Create Hugo event entries from URLs or sparse notes.")
    subparsers = parser.add_subparsers(dest="command", required=True)

    probe = subparsers.add_parser("probe", help="Gather metadata and emit an event spec as JSON.")
    probe.add_argument("--repo-root", required=True, help="Repository root.")
    probe.add_argument("--input", action="append", required=True, help="Input URL or free-form note. Repeatable.")
    ...
    write = subparsers.add_parser("write", help="Write an event page from a confirmed JSON spec.")
    write.add_argument("--repo-root", required=True, help="Repository root.")
    write.add_argument("--spec-json", required=True, help="Path to the JSON spec emitted by `probe`.")
    return parser
